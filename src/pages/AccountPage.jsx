...
import {
  CalendarDays, Layers, Database, ShoppingCart, Sparkles, Zap, Crown,
  Loader2, AlertCircle, ArrowRight, Plus, Minus, Receipt,
} from 'lucide-react'
import { useAuth } from '../context/AuthContext'
import {
  mySubscription, getUsage, getConfig, createOrder, verifyOrder,
...
export default function AccountPage() {
  ...
  const buyPackets = async () => {
    ...
    try {
      if (!config?.configured) throw new Error('Payments are not configured. Contact admin.')
      const ok = await loadRazorpay()
      if (!ok) throw new Error('Could not load Razorpay.')

      const order = await createOrder({ kind: 'addon', packets })
      if (!order.status) throw new Error(order.message || 'Order creation failed')
      const o = order.data
      ...
      rzp.on('payment.failed', (r) => {
        setError(r?.error?.description || 'Payment failed'); setBusy(false)
      })
      rzp.open()
    } catch (e) {
      setError(e.message || 'Could not start checkout'); setBusy(false)
    }
  }
    ...
    <div className="max-w-5xl mx-auto px-6 py-10 text-white">
      ...
      <div className="card p-6 mb-6">
        <div className="flex flex-wrap items-start gap-4 justify-between">
          <div className="flex items-center gap-3">
            <span className={`w-12 h-12 rounded-xl flex items-center justify-center
              ${planCode === 'premium' ? 'bg-purple-500/20 text-purple-300'
                : planCode === 'basic' ? 'bg-brand-500/20 text-brand-400'
            ...
            <div>
              ...
              <div className="text-2xl font-black">
                {sub?.plan_name || (planCode ? planCode.charAt(0).toUpperCase() + planCode.slice(1) : 'No active plan')}
              </div>
              {sub?.period_end && (
                <div className="text-xs text-white/40 flex items-center gap-1.5 mt-1">
                  <CalendarDays size={12} />
                  Renews / expires on {new Date(sub.period_end).toLocaleDateString()}
                </div>
              )}
            </div>
          </div>
        ...
        {sub && (
          <div className="grid sm:grid-cols-3 gap-3 mt-6">
            <Stat icon={Database} label="Datasize"
              value={sub.datasize_mb >= 1024 ? `${(sub.datasize_mb/1024).toFixed(0)} GB` : `${sub.datasize_mb} MB`} />
            <Stat icon={Layers} label="Workspaces"
              value={`${sub.workspaces + (sub.extra_workspaces || 0)}`}
              hint={sub.extra_workspaces ? `(+${sub.extra_workspaces} addon)` : null} />
            <Stat icon={Receipt} label="Last payment"
      ...
      {sub && sub.plan_code !== 'free' && (
        <div className="card p-6">
          ...
          <p className="text-sm text-white/50 mb-5">
            Each packet adds <strong className="text-white">+10 workspaces</strong> to your current plan
            for <strong className="text-white">₹500</strong>.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="inline-flex items-center bg-white/5 border border-white/10 rounded-xl">
              ...
              <input
                type="number" min="1" max="100" value={packets}
                onChange={(e) => setPackets(Math.max(1, Math.min(100, Number(e.target.value) || 1)))}
            ...
            <div className="text-sm text-white/60">
              {packets} packet{packets > 1 ? 's' : ''} = +{packets * 10} workspaces ·
              <span className="text-white font-semibold"> ₹{packets * 500}</span>
            </div>
      ...
      {!sub && (
        <div className="card p-6 text-center">
          <p className="text-white/60 mb-4">You don't have an active plan yet.</p>
  ...
  return (
    <div className="bg-white/[0.03] border border-white/5 rounded-xl px-4 py-3">
      ...
      <div className="text-lg font-black mt-1">
        {value} {hint && <span className="text-xs font-normal text-white/40">{hint}</span>}
      </div>
    </div>
  )
}
