...
import {
  Check, Loader2, Sparkles, Zap, Crown, AlertCircle, ArrowLeft,
} from 'lucide-react'
import { useAuth } from '../context/AuthContext'
import {
  listPlans, subscribeFree, createOrder, verifyOrder, getConfig,
...
export default function PricingPage() {
  ...
  useEffect(() => {
    Promise.all([listPlans().catch(() => null), getConfig().catch(() => null)])
      .then(([p, c]) => {
  ...
  const handleFree = async () => {
    setError(''); setSuccess(''); setBusy('free')
    try {
  ...
  const handlePaid = async (planCode) => {
    setError(''); setSuccess(''); setBusy(planCode)
    try {
      ...
      const order = await createOrder({ kind: 'subscription', plan_code: planCode, country })
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10 space-y-3">
          ...
          <p className="text-white/50 max-w-xl mx-auto">
            Start free, scale when you're ready. Switch or top-up workspaces any time.
          </p>
              ...
              <div key={plan.code}
                ...
                <div className="flex items-center gap-3 mb-2">
                  <span className={`w-9 h-9 rounded-lg flex items-center justify-center
                    ${plan.code === 'premium' ? 'bg-purple-500/20 text-purple-300'
                      : plan.code === 'basic' ? 'bg-brand-500/20 text-brand-400'
                ...
                <ul className="space-y-2 mt-4 mb-6 text-sm">
                  <Feat>Datasize: <strong className="text-white">{plan.datasize_label || `${plan.datasize_mb} MB`}</strong></Feat>
                  <Feat>Workspaces: <strong className="text-white">{plan.workspaces}</strong></Feat>
                  {plan.addon_packet_inr ? (
                    <Feat>Addon: ₹{plan.addon_packet_inr} for +10 workspaces</Feat>
                  ) : null}
                  <Feat>Inputs: {(plan.inputs || []).join(', ')}</Feat>
                  {(plan.reports || []).map((r, i) => <Feat key={i}>{r}</Feat>)}
                  {plan.has_market_research && <Feat>Gartner & Nielson market research</Feat>}
                  {plan.has_deep_insights && <Feat>Deep Insights add-on</Feat>}
                </ul>

                <button
                  disabled={isCurrent || loading}
                  onClick={() => isFree ? handleFree() : handlePaid(plan.code)}
        ...
        <p className="text-center text-xs text-white/30 mt-10">
          Secure checkout by Razorpay. Taxes (where applicable) added at payment.
        </p>
      </div>
    </div>
  )
